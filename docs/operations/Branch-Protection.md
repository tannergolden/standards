...
> - **`protect-release-tags.json`** covers the `v*.*.*` **tag** namespace: once minted, an exact version tag can never be moved, by anyone, so what it resolved yesterday it resolves today. It can be deleted, because `release.yml` prunes every version but the latest after each release; anyone who wants a pin that outlives the next release pins a commit SHA. Creating new tags (which the release automation does) stays unrestricted, and an admin can temporarily disable the ruleset in Settings for genuine emergencies.
> - **The pattern is `v*.*.*`, not `v*`, and that is load-bearing.** A moving major tag like `v1` is _supposed_ to be re-pointed at every release in its line - that is the entire mechanism by which a fix reaches consumers. `v*` would protect it too and quietly break every future release. Two dots is what separates the version, never re-pointed, from the movable major.
>
