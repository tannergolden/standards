...
An action, a reusable workflow or a kit is not installed; it is pinned, and the pin is a **moving major tag**: `@v1` follows every release in the line, so a fix reaches every consumer on their next run with nobody editing a workflow. `release-publish.yml` cannot produce that tag - its information-rich `vX.Y.Z-branch.<stamp>` is right for a deliverable that is downloaded and wrong for one that is resolved - so a repository consumed by tag calls **`release.yml`** from a stub of its own instead. One dispatch names `vX.Y.Z`; the run refuses a commit that is not already on the default branch and a version that does not move forward, proves the candidate with the files a consumer resolves (`required-files`) and the check that shows it works (`check-command`), then tags the version, force-moves the major, publishes the release with generated notes, and prunes every release it superseded, page and tag, so a repository consumed by tag carries exactly two: the major every stub pins and the version it points at. Tags are lightweight, so what GitHub verifies is the commit. A version pin lasts until the next release; anyone who wants exactness for good pins a commit SHA, which nothing here deletes.
